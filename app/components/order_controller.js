...
export default class extends Controller {
  ...
  calculateTotal() {
    const subtotal = this.itemSubtotalTargets
      .map((t) => Number(t.value))
      .reduce((s, a) => s + a, 0)
    this.subtotalTarget.innerHTML = (
      Math.round((subtotal + Number.EPSILON) * 100) / 100
    ).toLocaleString()

    const subtotalWithMultiplier =
      (Number(this.multiplierTarget.value) / 100) * subtotal
    this.subtotalWithMultiplierTarget.innerHTML =
      subtotalWithMultiplier.toLocaleString()
