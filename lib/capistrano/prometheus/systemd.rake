...
namespace :prometheus do
  namespace :systemd do
    ...
    task :generate_config_locally do
      ...
      run_locally do
        File.write('prometheus.service', git_plugin.compiled_template_prometheus("prometheus.service", fake_role.new("example.com")).string)
      end
    end
    ...
    task :enable do
      on roles(fetch(:prometheus_role)) do
        ...
        if fetch(:prometheus_systemctl_user) != :system && fetch(:prometheus_enable_lingering)
          execute :loginctl, "enable-linger", fetch(:prometheus_lingering_user)
        end
      end
    end
